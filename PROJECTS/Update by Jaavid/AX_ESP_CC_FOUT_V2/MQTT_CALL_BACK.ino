...
void callback(char* topic, byte * payload, unsigned int length)
{
  ...
  if(strstr(topic,sendtopic_ESMesh)!=NULL)
  {
    message_from_outside_node(payload,length);
  }
  else if(strstr(topic,sendtopic_WSMesh)!=NULL)
  {
    message_from_inside_node(payload,length);
  }
  else if(strstr(topic,sendtopic_app)!=NULL)
  {
