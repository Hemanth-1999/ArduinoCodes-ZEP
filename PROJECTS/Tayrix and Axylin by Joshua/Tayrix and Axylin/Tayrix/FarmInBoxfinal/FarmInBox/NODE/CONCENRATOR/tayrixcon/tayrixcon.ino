...
void onReceive(int packetSize)
{
  ...
  if ( sender == 0XBB )
  {
   ...
   int soilmoisturepert =  map(soilMoistureValue, 4000, 750, 0, 100);
   if(soilmoisturepert<=100)
   {
    soilmoisturepercent = soilmoisturepert;
   }
    
    soilMoistureValue_2 = v_2.toInt();
    
  int  soilmoisturepent_2 = map(soilMoistureValue_2, 4000, 750, 0, 100);
    if(soilmoisturepent_2<=100)
    {
      soilmoisturepercent_2 = soilmoisturepent_2;
    }
    temperature = t.toInt();
    pressure = pr.toInt();
    humidity = h.toInt();
