...
StaticJsonDocument<200> doc; //for testing http request
...
void setup() {
  ...
  //tempstring="Bharat Pi 4G Module Testing";
  ...
  modem.sendAT("+HTTPDATA=" + String(tempstring.length()) + ",20000"); //length of the body (data) and max time required in milliseconds to input the data
  ...
  //JSON tempstring or it can even be a plain text.
  //If sending json data then format tempstring accordingly before sending else it might result in post error
  modem.streamWrite(tempstring);
  if (modem.waitResponse(10000L) != 1) {
...
void sendDataMQTT() {
  ...
  doc["Wish"] = "Hi";
  serializeJson(doc, tempstring); //Convert data to json format
}
