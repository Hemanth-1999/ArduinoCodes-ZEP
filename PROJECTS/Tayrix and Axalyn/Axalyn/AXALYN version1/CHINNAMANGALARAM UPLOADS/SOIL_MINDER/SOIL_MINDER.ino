...
char hostname[40] = "164.52.223.248";//"cea.axalyn.com";
char port[40] = "1883";
...
void sendDataMQTT() {
  ...
  metrics["vpd"] = (float)(E_VPD) / 10;
  
  int E_HI = (int)(HI * 10);
  metrics["hi"] = heatIndex;
  
  metrics["co2_v"] = sensorValue1;
  
  int E_ETo = (int)(ETo * 10);
  metrics["et_v"] = (float)(E_ETo) / 10;
  
  int E_BP = (int)(ES_BP * 10);
  metrics["bp"] = pres;
  metrics["uv"] = uvIntensity * 10;
...
void setup() {
  ...
  if (!wifiManager.autoConnect("ES_NODE")) {
