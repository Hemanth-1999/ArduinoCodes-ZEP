...
#define EC_PIN A0                       // pH meter Analog output to Arduino Analog Input 0
...
Task t1(10000,  TASK_FOREVER, &GetSensorReading, &r);//getNutrientPHavg
Task t5(10000,  TASK_FOREVER, &AutoFun, &r, false);//AutoFun
Task t6(10000,  TASK_FOREVER, &scheduleFun, &r, false);//scheduleFun
Task t12(10000,  TASK_FOREVER, &nexVal, &r);
Task t16(45000,  TASK_FOREVER, &calibartion, &r);
Task t20(10000,  TASK_FOREVER, &setTimeNex, &r);
Task t21(10000,  TASK_FOREVER, &runTimeNex, &r);
Task t15(3000,  TASK_FOREVER, &manualread, &r);
Task t18(4000,  TASK_FOREVER, &eventmode, &r);
...
void scheduleFun() {
  Serial.print("scheduleFun() is running ");
  digitalWrite(LEDGREEN, HIGH);
...
void calibartion() {
  ...
  ph.calibration(voltagePH, temperature);     //PH calibration process
  ec.calibration(voltageEC, temperature);     //EC calibration process
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    // Read the incoming data as a character
    char incomingData = Serial.read();

    // Proceed with printing or calling functions based on availability
    Serial.println("Data received. Now printing automatically:");

    // Call your functions or print data automatically
    if (Resetfun == true) {
