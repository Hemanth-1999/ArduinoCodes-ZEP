...
int oftmin() {
  char *ui = strtok(buff, "g");//on time hr val
  str = ui;
  str.remove(0, 5);
  oftMin = str.toInt();
  return (oftMin);
...
DepthIrrMode_shift01=true;
 ...
 int ZoneAMin =Irrigation_time;   
  ...
  Zone01_MainLinePump.begin( minToMills, ZPumpStatus);
...
Serial.println(Urea_fertilizer);
delay(500);
per_nitrogen=othr();
delay(500);
...
if ((data_from_display.substring(0, 5) == "slot1"))
{
  Serial.println("slot1");
  int Slot01_DIModeON_SetTime_Hr =oftmin1();
  int Slot01_DIModeON_SetTime_min =ofthr();
  Serial.print("hai:");
...
if ((data_from_display.substring(0, 5) == "slot2"))
{
  Serial.println("slot2");
  int Slot02_DIModeON_SetTime_Hr =oftmin1();
  int Slot02_DIModeON_SetTime_min =ofthr();
  Serial.print("hai:");
