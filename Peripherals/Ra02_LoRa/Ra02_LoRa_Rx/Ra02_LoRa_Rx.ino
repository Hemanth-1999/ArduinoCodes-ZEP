...
bool newData = false;
bool initializeLoRa();
bool slave1Ack = false;
bool slave2Ack = false;
...
const long responseTimeout = 5000; // Timeout waiting for slave responses
...
void loop() {
  ...
  if (currentMillis - pageChangeMillis >= pageInterval) {
    ...
    currentPage = (currentPage % 4) + 1;  // Cycle through pages 1, 2, 3, 4
